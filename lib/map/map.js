...
module.exports = {
    ...
    create: function (context) {
        ...
        return {
            'CallExpression[callee.object.name="_"][callee.property.name="map"]'(node) {
                if (node.type === 'CallExpression') {
                    ...
                    if (hasTwoArguments(node)) {
                        ...
                        if (node.parent.test && node.parent.test.type == 'BinaryExpression') {
                            return;
                        }
            ...
            'VariableDeclarator[init.type="CallExpression"][init.callee.object.name="_"][init.callee.property.name="map"]'(node) {
                ...
                if (hasTwoArguments(callExpression)) {
                    ...
                    const sourceCode = context.getSourceCode();
                    const collection = sourceCode.getText(callExpression.arguments[0]);
                    const callback = sourceCode.getText(callExpression.arguments[1]);
                    if (isFirstArgumentArray(callExpression)) {

                        errorReport.fix = function (fixer) {
                            const varName = node.id.name;
                            return fixer.replaceText(node, `${varName} = ${collection}.map(${callback})`)
                        }

                    } else {
                        errorReport.fix = function (fixer) {
                            return fixer.replaceText(callExpression,
                                `${collection} instanceof Array ? ${collection}.map(${callback}) : _.map(${collection}, ${callback})`
                            )
                        }
                    }
